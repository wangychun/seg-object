...
//		obj_basic_new = classify_cloud(Grid , obj_basic,mark_to_checked);
...
//			cout <<"nark" << mark << ":" << obj_basic_new[mark].obj_ids.size() << endl;
			...
			*cloud_marked_joint += *cloud_marked_color;
			...
			cout <<"mark" << mark << ":" << obj_basic[mark-2].obj_ids.size() << endl;
			save_mark_color_cloud (cloud_marked_color, std::to_string(mark+2));
		}
