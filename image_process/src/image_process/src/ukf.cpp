...
void UnscentedKF::initialize(long long id, const sensors_fusion::ObjectTrack& obj, double time_stamp)
{
  ...
  // Track object is valid 3D object or not
  is_valid_ = obj.is_valid;
...
//  track.hist.bad_age++;
  ...
  // Record the history trajectory
  geometry_msgs::Point track_point;
  track_point.x = detected_objects.world_pos.point.x;
  track_point.y = detected_objects.world_pos.point.y;
  track_point.z = detected_objects.world_pos.point.z;
  track.hist.trajectory.points.push_back(track_point);

  // Store the original world position from detection module
  track.sta.world_origin_pos= detected_objects.world_pos;
...
void UnscentedKF::reset()
{
  state_ = TrackState::Tentative;

  // Reset track object is not valid 3D track object
  is_valid_ = false;
